...
read_csv_time <- system.time({
  df_read_csv <- "~/mimic/hosp/admissions.csv.gz" %>% read_csv()
})[3]
read_csv_time
```

```{r}
read.csv_time <- system.time({
  df_read_csv_base <- "~/mimic/hosp/admissions.csv.gz" %>% read.csv()
})[3]
read.csv_time
```
...
fread_time <- system.time({
  dt_fread <- "~/mimic/hosp/admissions.csv.gz" %>% fread()
})[3]
fread_time
```
```{r}
class(df_read_csv)
class(df_read_csv_base)
class(dt_fread )
```

```{r}
object_size(df_read_csv)
object_size(df_read_csv_base)
object_size(dt_fread )
```

The `fread` function is fastest.
...
```{bash}
cd  ~/Desktop/203B/hw-biostat-203B-2024/hw2
...
system.time({
  labevents <- arrow::open_dataset(
    "~/Desktop/203B/hw-biostat-203B-2024/hw2/labevents.csv", 
    format = "csv"
  ) %>%
...
```{r}
labevents_csv <- arrow::open_dataset("~/Desktop/203B/hw-biostat-203B-2024/hw2/labevents.csv", format = "csv")
arrow::write_dataset(labevents_csv, "~/Desktop/203B/hw-biostat-203B-2024/hw2/labevents.parquet")
```
...
system.time({
  labevents_parquet <- arrow::open_dataset(
    "~/Desktop/203B/hw-biostat-203B-2024/hw2/labevents.parquet") %>%
...
```{r}
sorted_labevents_parquet <- labevents_parquet %>%
  arrange(subject_id, charttime, itemid)
head(sorted_labevents_parquet, 10)
```

Parquet is  an open source, column-oriented data file format, designed to store and retrieve information in a really smart way. Parquet organizes data neatly and makes it quicker for your computer to find and work with the information you need, while also saving space. It's like having a super-powered filing cabinet that makes handling big data projects and analytics a breeze.
...
```{r}
  chartevents <- arrow::open_dataset(
    "~/Desktop/203B/hw-biostat-203B-2024/hw2/chartevents.csv", 
    format = "csv"
  ) %>%
